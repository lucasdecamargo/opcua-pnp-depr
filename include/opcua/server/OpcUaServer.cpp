        ...
        logger(std::move(_logger)) 
{
    ...
    pnp::log::LoggerFactory::setLoggerLevel(loggerUaServer, settings["logging"]["level"]["opcua"]);
    loggerUaClient = logger->clone(logger->name() + "-ua-client");
    pnp::log::LoggerFactory::setLoggerLevel(loggerUaClient, settings["logging"]["level"]["opcua"]);
...
OpcUaServer::~OpcUaServer() {
    ...
    std::lock_guard<std::recursive_mutex> lc(pnp::opcua::serverMutex);
...
UA_StatusCode OpcUaServer::init(
    ...
    pnp::opcua::UA_Server_onServerAnnounce(server, onLdsData);
...
LockedServer OpcUaServer::getLocked() {
    return LockedServer{server, pnp::opcua::serverMutex};
}
